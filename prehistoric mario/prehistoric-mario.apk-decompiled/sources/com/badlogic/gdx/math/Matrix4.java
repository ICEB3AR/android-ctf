...
public class Matrix4 implements Serializable {
    ...
    /*  JADX ERROR: Type inference failed with exception
        jadx.core.utils.exceptions.JadxOverflowException: Type update terminated with stack overflow, arg: (r1v0 ?? I:??[int, float][])
    ...
    public Matrix4 avg(Matrix4 matrix4, float f) {
        ...
        float f2 = 1.0f - f;
        setToScaling(tmpVec.scl(f).add(tmpForward.scl(f2)));
        rotate(quat.slerp(quat2, f2));
        setTranslation(tmpUp.scl(f).add(right.scl(f2)));
        return this;
    }

    public Matrix4 avg(Matrix4[] matrix4Arr) {
        float length = 1.0f / ((float) matrix4Arr.length);
